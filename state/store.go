...
var (
	...
	prefixValidatorsKey      = "validatorsKey:"
	prefixConsensusParamsKey = "consensusParamsKey:"
	prefixABCIResponsesKey   = "abciResponsesKey:"
	prefixStateKey           = "stateKey"
)
...
func validatorsKey(height int64) []byte {
	...
	if legacyKey == true {
		...
		key = []byte(prefixValidatorsKey + strconv.FormatInt(height, 10))

	} else {

		key = encodeKey(prefixValidators, height)
	}
...
func consensusParamsKey(height int64) []byte {
	...
	if legacyKey == true {
		...
		key = []byte(prefixConsensusParamsKey + strconv.FormatInt(height, 10))

	} else {
		key = encodeKey(prefixConsensusParams, height)
	}
...
func abciResponsesKey(height int64) []byte {
	...
	if legacyKey == true {
		...
		key = []byte(prefixABCIResponsesKey + strconv.FormatInt(height, 10))

	} else {

		key = encodeKey(prefixABCIResponses, height)
...
//default is legacy key with legacy key prefixes, eg "abciResponseKey:1000000"
func SetLegacyKey(isLegacyKey bool) {
