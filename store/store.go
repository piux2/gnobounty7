...
import (
	...
	"github.com/piux2/gnobounty7/keymigrate"
...
func NewBlockStore(db dbm.DB) *BlockStore {
	bs := &BlockStore{db}
	if bs.isLegacyKey() == false {

		panic("the blockstore database has migrated to new keys. This program only support legacy")
	}
	return bs
}
func (bs *BlockStore) DB() dbm.DB {

	return bs.db

}
func (bs *BlockStore) isLegacyKey() bool {

	ldb := bs.db.(*dbm.GoLevelDB)

	// loop through ASCII a-z and A-Z as prefixes to find all keys in the database.
	for j := 56; j <= 122; j++ {
		a := rune(j)
		i := ldb.DB().NewIterator(util.BytesPrefix([]byte(string(a))), nil)
		...
		if strings.Contains("BH C H P SC", prefix) == true {
...
func (bs *BlockStore) SetLegacyKey(isLegacyKey bool) {

	legacyKey = isLegacyKey
}
...
var (
	...
	prefixBlockMetaKey   = "H:"
	prefixBlockPartKey   = "P:"
	prefixBlockCommitKey = "C:"
	prefixSeenCommitKey  = "SC:"
	prefixBlockHashKey   = "BH:"
)

var legacyKey bool = true
...
func blockMetaKey(height int64) []byte {
	...
	if legacyKey == true {
		...
		key = []byte(prefixBlockMetaKey + strconv.FormatInt(height, 10))
...
func decodeBlockMetaKey(key []byte) (int64, error) {
	var err error
	var height int64
	var remaining string

	if keymigrate.CheckKeyType(key).IsLegacy() {
		key, err = keymigrate.MigrateKey(key)
		if err != nil {

			return -1, fmt.Errorf("can not parse key: %v", err)
		}
	}
...
func blockPartKey(height int64, partIndex int) []byte {
	...
	if legacyKey == true {
		...
		key = []byte(prefixBlockPartKey + strconv.FormatInt(height, 10) + ":" + strconv.Itoa(partIndex))
	} else {
...
func blockCommitKey(height int64) []byte {
	...
	if legacyKey == true {
		...
		key = []byte(prefixBlockCommitKey + strconv.FormatInt(height, 10))
	} else {
...
func blockHashKey(hash []byte) []byte {
	...
	if legacyKey == true {
		...
		key = []byte(prefixBlockHashKey + string(hash))
	} else {
